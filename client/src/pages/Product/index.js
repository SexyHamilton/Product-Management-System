...
export default function ProductList() {
  ...
  const handleAddToCart = (userId, product_id) => {
    dispatch(addProductToCartAction({ userId: userId, productId: product_id }));
          ...
          <Skeleton title={false} loading={status === "pending"} active>
            ...
            >
              ...
              {user.id && item.user && user.id === item.user._id ? (
                <div>
                  {initialCartItemQuantityState[item._id] === 0 ? (
                    <button
                      className={classes.button}
                      onClick={() => handleAddToCart(user.id, item._id)}
                    >
                      Add
                    </button>
                  ) : (
                    <div className={style.list_div_add_and_delete}>
                      <Tooltip title="Remove">
                        <Button
                          className={style.add_delete_btn}
                          onClick={() => handleDropFromCart(user.id, item._id)}
                        >
                          <RemoveIcon
                            style={{
                              color: "white",
                      ...
                      <p className={style.btn_quantity_home}>
